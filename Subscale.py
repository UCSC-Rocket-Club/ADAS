...
def num_solver(thrust_profile, rocket_mass, motor_mass, propellant_mass, time_res, temp, burn_time, max_deploy, t_start, t_deploy, plots, drag_f) :
    ...
    L0 = 0.0065         # [K/m] Standard temperature lapse rate
    ...
    for i in range (0, len(t.arr))
        ...
        if MECO == False :
            if (ai <= (-g + buffer_acc) or t.arr[i] > t.burn) :
                t_MECO = t.arr[i]
